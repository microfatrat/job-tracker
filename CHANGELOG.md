...
- **阶段变更可以指定日期**：详情面板的「阶段流转」里新增「阶段日期」日历选择器（默认今天），
  推进到下一阶段、点阶段按钮切换、标记拒绝/放弃都按这一天写入历史，
  补录“一面其实是上周三”这类记录不再需要事后手改 JSON。日期不是今天时，
  选择器旁边会变成橙色提醒（“将按 2026-09-13 记录阶段变更”）。
- **阶段历史逐条改日期**：阶段历史里每条非「已投递」事件右侧新增「改日期」按钮，
  弹窗里用日历改成正确日期，保存后当前阶段与 updated_at 都不受影响；
  「已投递」事件的日期仍跟随表单里的「投递日期」。

### 变更

- 阶段事件日期有三条约束，违反时提示并拒绝写入：不早于投递日期、不早于上一条事件（新事件）、
  不晚于今天；改已有事件时区间还要落在相邻两条事件之间。
  这样不会再出现“面试日期早于投递日期”导致的负数周期，也不会把历史顺序弄乱。
- 阶段变更在写入后统一按日期排序（此前依赖调用方按时间顺序追加），
  补录更早的事件时「阶段历史」列表与「撤销上次阶段变更」的行为仍然一致。
- 详情面板「阶段流转」多了一行日期选择器，docs/screenshots/applications.png 已同步更新。

### 测试

- 新增 3 个单元测试（共 30 个）：新阶段变更的日期区间校验、历史事件改日期的区间与排序、
  补录更早事件后历史仍按日期有序。
- 新增端到端脚本（本机 WSLg，不在仓库里）：推进阶段按指定日期写入、阶段历史改日期，
  断言全部读磁盘 JSON。
